...
export const removeBookmark = async (id) => {
  const { data } = await supabase.from("bookmarks").delete().eq("temp_id", id);
  return data;
};
...
export const newFolder = async (folder) => {
  ...
  const folderWithId = {
    ...folder,
    temp_id: folder.temp_id ? folder.temp_id : uuidv4(),
  };
  // TODO: add error handling
  const { data, error } = await supabase.from("folders").insert([folderWithId]).single();
  return data;
};
